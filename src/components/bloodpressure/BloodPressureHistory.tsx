...
export default function BloodPressureHistory({ records, onDelete, onExport }: Props) {
  if (records.length === 0) {
    return null
  }
        ...
        <table className="table table-hover shadow-sm" style={{ borderRadius: '10px', overflow: 'hidden' }}>
          <thead style={{ backgroundColor: '#6610f2', color: 'white' }}>
            <tr>
              ...
              <th scope="col" className="text-center">Systolic</th>
              <th scope="col" className="text-center">Diastolic</th>
          ...
          <tbody>
            {records.map((record, index) => (
              <tr key={record.id}>
                ...
                <td className="text-center">
                  <span className="badge" style={{ fontSize: '14px', backgroundColor: '#E6681A' }}>
